...
def dfs(directory : dict) -> int:
    ...
    for k, v in directory.items():
        if isinstance(v, dict):
            ...
            total_current += v
    if total_current + all_directory_content <= 100000:
        ans += total_current + all_directory_content
    if total_current + all_directory_content >= min_removed_one:
        ans2 = min(ans2,total_current + all_directory_content)
    return total_current + all_directory_content
    ...
    while current_command < len(lines):
        ...
        if chunked[1] == 'cd':
            ...
            while current_command < len(lines) and lines[current_command][0] != '$':
                ...
                current_dict.current[chunked_file[1]] = dict() if chunked_file[0] == 'dir' else int(chunked_file[0])
                current_command += 1
